...
export default function HomeScreen() {
  return (
    <section className={styles.container} data-testid="home-root">
        ...
        <main className={styles.main}>
          ...
          <section className={`${styles.userCard} ${styles.softCard}`} aria-label="ユーザー情報">
            <div className={styles.userHeader}>
              <div className={styles.userMeta}>
                <div className={styles.avatarWrap}>
                  <Image src={user.avatar} alt={`${user.name} のアバター`} width={48} height={48} className={styles.avatar} />
                </div>
                <div>
            ...
            <ul className={styles.kpis} role="list">
              <li className={styles.kpiItem}><div className={styles.kpiNum}>{user.streakDays}</div><div className={styles.kpiLabel}>連続学習</div></li>
              <li className={styles.kpiItem}><div className={styles.kpiNum}>{user.totalHours}</div><div className={styles.kpiLabel}>累計時間</div></li>
              <li className={styles.kpiItem}><div className={styles.kpiNum}>{user.cleared}</div><div className={styles.kpiLabel}>クリア数</div></li>
            </ul>
          </section>
          ...
          <section className={`${styles.lessonCard} ${styles.softCard}`} aria-label="学習中のレッスン">
            ...
            <div className={styles.lessonCta}>
              <Button variant="primary" size="large" className={styles.compactBtn}>続ける</Button>
            </div>
          </section>
          ...
          <div className={`${styles.listStack} ${styles.softCardList}`}>
            {reviewLessons.map((l, idx) => (
              <article key={l.id} className={`${styles.itemRow} ${idx > 0 ? styles.rowDivider : ''}`}>
                <div className={styles.itemIcon}><Image src={l.icon} alt="" width={35} height={35} /></div>
                <div className={styles.itemBody}>
                  <div className={styles.itemBadge}>{l.badge}</div>
                  <div className={styles.itemTitle}>{l.title}</div>
                </div>
                <div className={styles.itemCta}><Button variant="primary" size="medium" className={styles.compactBtn}>復習する</Button></div>
              </article>
            ))}
          </div>
          ...
          <div className={`${styles.listStack} ${styles.softCardList}`}>
            {bookmarks.map((l, idx) => (
              <article key={l.id} className={`${styles.itemRow} ${idx > 0 ? styles.rowDivider : ''}`}>
                ...
                <div className={styles.itemCta}><Button variant="primary" size="medium" className={styles.compactBtn}>学習する</Button></div>
              </article>
            ))}
          </div>
        </main>
          ...
          <section className={`${styles.card} ${styles.softCard}`} aria-label="学習カレンダー">
            ...
            <div className={styles.calendar}>
              <div className={styles.weekRow}>
                {calendar.weekLabels.map((w) => <span key={w} className={styles.weekCell}>{w}</span>)}
              </div>
              <div className={styles.dayGrid}>
                {calendar.days.map((d, i) => {
                  ...
                  return (
                    <span key={d} className={`${styles.dayCell} ${hit ? styles.dayHit : ''} ${hideOnMobile}`}>{d}</span>
                  );
                })}
              </div>
            </div>
            ...
            <ul className={styles.activityList} role="list">
              {activities.map((a, idx) => (
                <li key={a.id} className={`${styles.activityItem} ${idx > 0 ? styles.rowDivider : ''}`}>
                  <Image src={a.avatar} alt={`${a.name} のアイコン`} width={32} height={32} className={styles.activityAvatar} />
